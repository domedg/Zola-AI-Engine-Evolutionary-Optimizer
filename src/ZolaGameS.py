...
class ZolaGUI:
    ...
    def __init__(self, game, player_types, time_out=3, player_names=None, advanced_team="Both"):
        ...
        red_role = "AI (Advanced)" if self.player_types.get("Red") == "ai" and self.advanced_team in ["Red", "Both"] else ("AI (Alpha)" if self.player_types.get("Red") == "ai" else "Umano")
        blue_role = "AI (Advanced)" if self.player_types.get("Blue") == "ai" and self.advanced_team in ["Blue", "Both"] else ("AI (Alpha)" if self.player_types.get("Blue") == "ai" else "Umano")
    ...
    def update_board(self):
        ...
        primo = f"[Inizia il {self.player_display_name(self.game.first_player)}] - " if self.current_index == 0 else ""
        self.status_label.config(
            text=f"{primo}Turno: {self.player_display_name(state.to_move)}{prompt}{extra}"
        )
    ...
    def play_turn(self):
        ...
        self.thinking_time[current_player] += turn_duration

        new_state = self.game.result(state, move)
        if new_state.last_move:
            new_state.last_move["duration_seconds"] = turn_duration
    ...
    def save_match_history(self):
        ...
        import datetime
        import os
        log_file = os.path.join(os.path.dirname(__file__), '../logs/match_history.json')
        ...
        winner = self.game.winner(self.latest_state())
        
        record = {
            "date": datetime.datetime.now().isoformat(),
            "red_player": self.player_names.get("Red", "Rosso"),
            "blue_player": self.player_names.get("Blue", "Blu"),
            "winner": winner,
            "total_time_seconds": self.total_game_time(),
            "red_think_time": self.thinking_time["Red"],
            "blue_think_time": self.thinking_time["Blue"],
            "moves": moves
        }
        ...
        try:
            ...
            print(f"Partita salvata correttamente in {log_file}")
...
def main():
    ...
    # Crea una mini-finestra invisibile per i prossimi dialoghi per evitare errori
    temp_root = tk.Tk()
    temp_root.withdraw()
